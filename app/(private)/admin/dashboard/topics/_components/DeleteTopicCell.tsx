...
import { deleteTopic } from "@/actions/topic/topicActions";
...
export default function DeleteTopicCell({ row, onSuccess }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await deleteTopic(row._id);

      if (res?.status) {
        ToastMessage.success({
          title: res?.message || "Topic deleted successfully!",
        });
        setOpen(false);
        onSuccess();
      } else {
        ToastMessage.error({
          title: res?.message || "Failed to delete topic",
        });
      }
    } catch {

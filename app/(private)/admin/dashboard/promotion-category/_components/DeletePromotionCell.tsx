...
import { deletePromotion } from "@/actions/promotion/promotionActions";
...
export default function DeletePromotionCell({ row, onSuccess }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await deletePromotion(row._id);

      if (res?.status) {
        ToastMessage.success({
          title: res?.message || "Promotion category deleted successfully!",
        });
        setOpen(false);
        onSuccess();
      } else {
        ToastMessage.error({
          title: res?.message || "Failed to delete promotion category",
        });
      }
    } catch {
